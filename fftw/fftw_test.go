...
func verifyImpulse2(t *testing.T, a *Array2, dim0, dim1 int, wantVal float64, impulse bool) {
	t.Helper()
...
func verifyImpulse3(t *testing.T, a *Array3, dim0, dim1, dim2 int, wantVal float64, impulse bool) {
	t.Helper()
	...
	for i := range dim0 {
		for j := range dim1 {
			if (i == int(freqX) || i == dim0-int(freqX)) &&
				...
				testAlmostEqual(t, real(a.At(i, j)), float64(dim0*dim1/4))
				testAlmostEqual(t, imag(a.At(i, j)), 0.0)
			} else {
		...
		for j := range dim1 {
			for k := range dim2 {
				if (i == int(freqX) || i == dim0-int(freqX)) &&
					...
					testAlmostEqual(t, real(a.At(i, j, k)), float64(dim0*dim1*dim2/8))
					testAlmostEqual(t, imag(a.At(i, j, k)), 0.0)
				} else {
...
func TestFFTAndIFFT1D(t *testing.T) {
	...
	for i := range input.Elems {
		input.Elems[i] = complex(float64(i+1), float64(-i))
	}
	...
	ifft := IFFT(fft)
	for i := range input.Elems {
		testAlmostEqual(t, real(ifft.Elems[i]), float64(n)*real(input.Elems[i]))
		testAlmostEqual(t, imag(ifft.Elems[i]), float64(n)*imag(input.Elems[i]))
	}
	...
	for i := range input.Elems {
		testAlmostEqual(t, real(out.Elems[i]), float64(n)*real(input.Elems[i]))
		testAlmostEqual(t, imag(out.Elems[i]), float64(n)*imag(input.Elems[i]))
	}
}

func TestFFT2ToAndIFFT2ToImpulse(t *testing.T) {
	t.Parallel()
	...
	out := NewArray2(dim0, dim1)
	FFT2To(out, input)

	verifyImpulse2(t, out, dim0, dim1, 1.0, false)

	IFFT2To(out, out)

	verifyImpulse2(t, out, dim0, dim1, float64(dim0*dim1), true)
}

func TestFFT3ToAndIFFT3ToImpulse(t *testing.T) {
	t.Parallel()
	...
	out := NewArray3(dim0, dim1, dim2)
	FFT3To(out, input)

	verifyImpulse3(t, out, dim0, dim1, dim2, 1.0, false)

	IFFT3To(out, out)

	verifyImpulse3(t, out, dim0, dim1, dim2, float64(dim0*dim1*dim2), true)
}

func TestFFTNAndIFFTNImpulse(t *testing.T) {
	t.Parallel()

	dims := []int{4, 3, 2}
	input := NewArrayN(dims)
	input.Set([]int{0, 0, 0}, complex(1, 0))

	fft := FFTN(input)
	for i := range fft.Elems {
		testAlmostEqual(t, real(fft.Elems[i]), 1.0)
		testAlmostEqual(t, imag(fft.Elems[i]), 0.0)
	}

	ifft := IFFTN(fft)
	for i := range ifft.Elems {
		want := 0.0
		if i == 0 {
			want = float64(dims[0] * dims[1] * dims[2])
		}

		testAlmostEqual(t, real(ifft.Elems[i]), want)
		testAlmostEqual(t, imag(ifft.Elems[i]), 0.0)
	}
	...
	for i := range out.Elems {
		...
		testAlmostEqual(t, real(out.Elems[i]), want)
